...
const Haeder = () => {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event) => {
      ...
      const menuTriggerKnowledge = document.querySelector(`li[data-knowledge-dropdown]`);
      const isClickInsideProductMenu = event.target.closest(`.${s.NavProductBlock}`) || event.target === menuTriggerProduct;
      const isClickInsideKnowledgeMenu = event.target.closest(`.${s.NavKnowledgeBlock}`) || event.target === menuTriggerKnowledge;
      console.log(isClickInsideProductMenu, isClickInsideKnowledgeMenu, previousActiveMenuRef.current, 'test1');
      ...
      previousActiveMenuRef.current = navProduct ? "project" : (navKnowledge ? "knowledgeBase" : null);
    };
      ...
      <haeder className={`${manrope.className}  ${s.haeder}`}>
        ...
        <nav className={s.nav}>
          <ul>
            ...
            <li onClick={() => setNavKnowledge(true)} >База знаний  <IoIosArrowDown />
            </li>
